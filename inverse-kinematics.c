...
void inverseKinematics(long double x,long double y,long double z,long double t[3][3],double angles[7]) /*results are stored in angles array */
{
    ...
    yc = y - d6*t[1][2];
    zc = z - d6*t[2][2];
    /* solve inverse kinematics for the first 3 angles */
    ...
    /* solve inverse kinematics for the final 3 angles, for details see:
    Modelling and Control of Robot Manipulators, Lorenzo Sciavicco and Bruno Siciliano chapter 2*/
    ...
    /* servo's can only move 180 degrees*/
    if (angles[4] > pi/2){
    ...
    /* some servo's are oriented different than the DH frames, so angles are adjusted for that here*/
    angles[1] = angles[1];
    angles[2] = angles[2];
    angles[3] = -(angles[3] - pi/2);
